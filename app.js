...
// add a mouse controlled constraint
    ...
    net.backboard.y = (canvas.height * 0.4) - net.backboard.h - ((ballDimensions.radius * 2) * backboardOffsetRatio);
...
// create game objects
    var ground = Bodies.rectangle(0, canvas.height, canvas.width * 2, (ballDimensions.radius * 2), {isStatic: true, render: { fillStyle: 'brown', visible: true}}),
        rightWall = Bodies.rectangle(canvas.width, 0, (ballDimensions.radius * 2), canvas.height * 2, {isStatic: true, render: {fillStyle: 'black', visible: true}}),
        garage = Bodies.rectangle(0, canvas.height, (ballDimensions.radius * 2), canvas.height * 1.2, {isStatic: true, render: {fillStyle: 'grey', visible: true}}),
        roof = Bodies.rectangle(0-ballDimensions.radius * 12, 0, (ballDimensions.radius * 2), canvas.height * 1.2, {isStatic: true, render: {fillStyle: 'black', visible: true}});
    Body.rotate(roof, -Math.PI/3);

    var rightRim = Matter.Bodies.circle(net.rightRim.x, net.rightRim.y, 2, {isStatic: true, render: {visible: true}}),
        leftRim = Matter.Bodies.circle(net.leftRim.x, net.leftRim.y, 2, {isStatic: true, render: {visible: true}});

    var ball = Matter.Bodies.circle(200, 200, ballDimensions.radius, {
        density: 1,
        restitution: 0.9,
        friction: 0.1,
        render: {fillStyle: '#ff5f9f'}
    });
    var ballStartingX = 0;
    var ballStartingY = 0;
    ...
    var leftChain = Composites.chain(leftBoxes, 0, 0, -0.5, 0, { stiffness: 1});
    var rightChain = Composites.chain(rightBoxes, 0, 0, -0.5, 0, { stiffness: 1});
